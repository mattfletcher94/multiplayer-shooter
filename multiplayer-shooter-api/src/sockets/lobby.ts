...
// Create LOBBY_IO
const LOBBY_IO = io.of('/lobby');

const rooms = {} as any;
const killHistories = {} as any;
const FPS = 30;
...
LOBBY_IO.use(authMiddleware).use(async function (socket : any, next : any) {

    // Make sure lobby still exists before connecting
    const lobby = await Game.findOne({
    ...
    if (lobby) {
        ...
        if (rooms[socket.lobby._id]) {
            ...
            var players = rooms[socket.lobby._id].filter((s : any) => {
    ...
    getPixels(socket.lobby.gameMap.mapCollisionTexturePath, function(err : any, pixels : any) {
        ...
        gameLoopId = gameloop.setGameLoop(function(delta : any) {
            ...
            if (!socket.lobby.hasStarted) {
                ...
                if (moment().isAfter(socket.lobby.gameStartDatetime)) {
                    socket.lobby.hasStarted = true;
                }

                // Calculate time until start
                socket.lobby.timeUntilStart = Math.abs(moment().diff(socket.lobby.gameStartDatetime, 'seconds')).toString();
            ...
            // Get players
            players = rooms[socket.lobby._id].map((s : any) => {
                return s.player
            });

            
            // If game has ended
            if (moment().isAfter(socket.lobby.gameEndDatetime)) {
                socket.lobby.hasEnded = true;
            }

            // Calculate time until end
            socket.lobby.timeUntilEnd = Math.abs(moment().diff(socket.lobby.gameEndDatetime, 'seconds')).toString();
        ...
        // On move
        socket.on('playerUpdate', (payload : any) => {
...
function calculatePosition (worldWidth : number, worldHeight : number, pixels : any, x : number, y: number, deltaX : number, deltaY : number, speed : number) : { x: number; y: number } {
...
function calculateBulletPosition (worldWidth : number, worldHeight : number, pixels : any, x : number, y: number, deltaX : number, deltaY : number) : { x: number; y: number } {
...
function randomPosition (worldWidth : number, worldHeight : number, pixels : any) : { x: number; y: number } {
...
function calculatePointBetween(x : number, y : number, x2 : number, y2 : number, percentage : number) : { x: number; y: number } {
    return {
        x: x + (x2 - x) * percentage,
        y: y + (y2 - y) * percentage
    }
